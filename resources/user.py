...
from flask import request, jsonify, make_response, render_template, abort
from schemas.user import UserSchema
from schemas.confirmation import ConfirmationSchema
from models.user import UserModel
from models.confirmation import ConfirmationModel
from marshmallow import ValidationError
from flask_jwt_extended import create_access_token, create_refresh_token, jwt_required, get_raw_jwt, get_jwt_identity,decode_token
        ...
        if user.is_correct_password(data["password"]):
            if user.confirmation[0] and user.confirmation[0].confirmed:
                access_token = create_access_token(
                    identity=user.email, fresh=True)
